...
import { getProductosGas, solicitarGas } from '../api/arriendo'

export default function CarritoGas({ responsables, onEnviado, onCancelar }) {
  ...
  useEffect(() => {
    getProductosGas().then((res) => setProductos(res.data)).finally(() => setCargando(false))
  }, [])
  ...
  const itemsCarrito = Object.entries(carrito).map(([id, cantidad]) => {
    ...
    return {
      producto_id: id,
      nombre: producto ? `${producto.tipo_display}${producto.nombre ? ' — ' + producto.nombre : ''}` : '',
      cantidad,
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-xs text-gray-400 italic -mt-2">
        * Los precios no incluyen despacho de gas.
      </p>
      ...
      {cargando ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {productos.map((p) => {
            ...
            return (
              <div key={p.id} className="bg-white rounded-lg shadow p-3 flex flex-col gap-1">
                <p className="text-sm font-bold text-dark leading-tight">{p.tipo_display}</p>
                {p.nombre && <p className="text-xs text-gray-500">{p.nombre}</p>}
                <p className="text-sm font-bold text-primary">${Number(p.precio).toLocaleString('es-CL')}</p>
                <div className="mt-auto pt-2">
                  {cantidad === 0 ? (
                    ...
                    <div className="flex items-center justify-between bg-primary/10 rounded">
                      <button onClick={() => cambiarCantidad(p.id, -1)} className="px-2.5 py-1 text-primary font-bold">−</button>
      ...
      {totalItems > 0 && (
        <div className="bg-white rounded-lg shadow p-4 flex flex-col gap-3 sticky bottom-4">
          <p className="text-sm font-bold text-dark">🛒 Tu carrito ({totalItems} {totalItems === 1 ? 'ítem' : 'ítems'})</p>
          <div className="flex flex-col gap-1 max-h-32 overflow-y-auto">
            {itemsCarrito.map((item) => (
              <div key={item.producto_id} className="flex justify-between text-sm text-gray-700">
                <span>{item.nombre} x{item.cantidad}</span>
                <span className="font-medium">${Number(item.precio * item.cantidad).toLocaleString('es-CL')}</span>
              </div>
            ))}
          </div>
          ...
          <p className="text-xs text-gray-400 italic">
            * No incluye despacho de gas.
          </p>

          <select value={responsableId} onChange={(e) => setResponsableId(e.target.value)} className="w-full border rounded p-2 text-sm">
